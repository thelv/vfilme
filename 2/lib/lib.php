<?php
	...
	function get_one_film($watch)
	{
		...
		$res=mysql_query("SELECT S.Id AS serial_id, S.name, S.seasons_number, E.season, E.series_number, R.series, F.watch, F.img, F.embed, F.description FROM serial S, season E, series R, film F WHERE R.film='$dbquery_watch' AND F.watch=R.film AND R.season=E.season AND E.serial=S.Id");
	...
	function get_seasons_number($serial,&$name)
	{
		...
		$res=mysql_query("SELECT seasons_number,name FROM serial WHERE serial.Id=$dbquery_serial");
		$result=mysql_fetch_assoc($res);
		$name=$result['name'];
		$name=html_line_out($name);
		mysql_free_result($res);
