  ...
  function MakeHTML($films,$HTMLfilm,$iwords)
  {
    ...
    if ((! ($film['img']==$lastimg)) || (strlen($lastdesc) <= strlen($film['description'])))
     { 
      ...
      echo "\r\n <a href='admin_edit.php?id=${film['Id']}' target='_blank' style='font-weight:bold;margin-top:2px;display:block' class=edit>edit</a> \r\n";
...
if (preg_match('|\Ahttp://vkontakte.ru/video-{0,1}[\d]+_[\d]+\Z|', $video_name)) { header("Location: video.php?video=$video_name"); die(); };
  ...
  echo "<html>\r\n<head>\r\n<title>В Фильме | Новые фильмы 2010 онлайн  | Скачать свежие новинки мирового кинопроката | То что сейчас идет в кино";
  if (! ($name=='')){echo "\"$name\" ";};
  if (! $info==''){echo "с \"$info\" ";};
  if (! $owner=='')
  {
  ...
  echo '</title>'."\r\n";
  echo '<meta name="keywords" content="скачать новые фильмы 2010,смотреть онлайн последние новинки кино"></meta>';
...
echo "\r\n<script language='JavaScript'>\r\n<!--\r\ndocument.getElementById('newsearch').className='opensearch';\r\n-->\r\n</script>\r\n";
  ...
  mysql_query("SELECT id FROM film WHERE $cond AND (flag=10 OR flag=2000)");    
  $nfilm=mysql_affected_rows($mysql);  
  $result=mysql_query("SELECT * FROM film WHERE (flag=10 OR flag=2000) GROUP BY img ORDER BY -`Id` LIMIT ".($page-1)*$ninpage.", $ninpage");    
  $pagesstr=showpages($page,(int)(($nfilm-1)/$ninpage)+1);
  echo $pagesstr;  
  echo "<div class='resultslabel'>\r\n<H1><span class=new>Киноновинки</span> Новинки мирового кинопроката / Новые фильмы 2010 (то что сейчас идет в кино) </H1></div>\r\n"; 
  echo '<form method=post action=admin_add.php target=_blank><input type=hidden name=request_is value=1><input type=submit value="add film"></form>';
  if ($nfilm==0 && (!($name==''))){
    echo $HTMLclaim[0];
    echo $name;
    echo $HTMLclaim[1];
