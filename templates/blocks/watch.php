<?php 
	if(count($watch['comments'])>10)
	{
		foreach($watch['comments'] as $key => $watch__comment)
		{
			if(mb_strlen($watch__comment,'utf8')<30)
			{
				unset($watch['comments'][$key]);
			}
		}
	}
...
$watch['fuck_embed']=iconv('WINDOWS-1251', 'UTF-8', $watch['embed']);
	...
	if($watch['serial_id'])
	{
		...
		$watch['viewout_series_text']=viewout_line($watch['series_text']);
		$watch['viewout_series_text']=viewout_line($watch['series_text']);
		$watch['viewout_serial_id']=viewout_line($watch['serial_id']);
		$watch['viewout_serial_alias']=urlencode($watch['serial_alias']);
	}
	...
	if($watch['PART']=='title'): 
		...
		{					
			echo "В Фильме | \"${watch['viewout_name']}\" -$watch__season_str ${watch['viewout_series_text']} $watch__series_word смотреть онлайн";
		}
	elseif($watch['PART']=='keywords'):
		...
		{						
			echo "${watch['viewout_name']}$watch__season_str ${watch['viewout_series_text']} $watch__series_word смотреть онлайн";
		}		
	elseif(! $watch['PART']):
?>
		<?php 
		...
		?>
		<div id=center_row style='margin-right:230px' class=watch_film>			
	<?php
		if(! $watch['serial_id']):
	?>
	...
	<?php
			...
			echo "
			<h1><a href='serial.php?serial=${watch['viewout_serial_alias']}'>${watch['viewout_name']}</a></h1>
			<h1 id=series_name><b>".($watch__show_season ? "<a href='serial.php?serial=${watch['viewout_serial_alias']}&season=${watch['viewout_season']}'>${watch['viewout_season']} сезон</a>" : "")." ${watch['viewout_series_text']} $watch__series_word</b></h1>
			";
	?>
	<?php endif; ?>		
			<div class=w_d>смотреть онлайн / <a href=download.php?film=<?php echo $watch['viewout_watch']; ?>>скачать</a></div>
			<div class=film_cont style='width:<?php echo preparevkiframe($watch['embed'],$watch['watch'],$watch['name'],$watch['blocked']); ?>px'>
				<div>
					<?php echo strpos($watch['fuck_embed'], 'table') ? $watch['fuck_embed'] : $watch['embed']; ?>
				</div>
						...
						<?php
							...
							for($i=1;$i<=$watch['seasons_number'];$i++)
							{
								...
								else
								{
									if($i==$watch['season']-1)
									{
										echo "<a class=\"page_link\" style='padding-right:0' href=\"serial.php?serial=${watch['viewout_serial_alias']}&season=$i\">$i</a> ";
									}
									else
									{
										echo "<a class=\"page_link\" href=\"serial.php?serial=${watch['viewout_serial_alias']}&season=$i\">$i</a> ";
									}
								}
							}
					...
					<span class="page_links">
						...
						<?php
							...
							foreach($watch['all_series'] as $watch__series_link)
							{
								...
								if($i==$watch['series'])
								{
									echo "<b class=\"page_link\">${watch__series_link['series_text']}</b> ";
								}
								else
								{
									if($i==$watch['series']-1)
									{
										echo "<a class=\"page_link\" style='padding-right:0' href=\"watch.php?film=".viewout_line($watch__series_link['watch'])."\">${watch__series_link['series_text']}</a> ";
									}
									else
									{
										echo "<a class=\"page_link\" href=\"watch.php?film=".viewout_line($watch__series_link['watch'])."\">${watch__series_link['series_text']}</a> ";
									}
								}
							}
...
<div id=mycomments>
	<script type="text/javascript">
	<!--
		document.write("<style>#mycomments{overflow:hidden}</style>");
	-->
	</script>
	<?php if(count($watch['comments'])): ?>
	<a id=mycomments_show onclick='this.parentNode.setAttribute("show","1")'>читать все комментарии к этому фильму</a>
	<ul>		
		<?php
