<?php
	...
	$serial['viewout_alias']=urlencode($serial['alias']);	
...
?>
	...
	<?php if($serial__show_season): ?>
			...
			<div class=pages>				
				...
				<?php
					...
					{
						echo " <a class=forw_back href='?serial=${serial['viewout_alias']}&season=".($serial['season']-1)."'>← предыдущий</a> ";
					}
					...
					{
						echo " <a class=forw_back href='?serial=${serial['viewout_alias']}&season=".($serial['season']+1)."'>следующий →</a> ";
					}					
				?>
				...
				<?php
					for($i=1;$i<=$serial['seasons_number'];$i++)
					{
						...
						{
							echo "<a class=page_link href='?serial=${serial['viewout_alias']}&season=$i' style='padding-right:0'>$i</a> ";
						}
						else
						{
							echo "<a class=page_link href='?serial=${serial['viewout_alias']}&season=$i'>$i</a> ";
						}						
					}					  
				?>
				</div>
	...
	<?php endif; ?>
			...
			<?php				
				...
				{
					$search_film['name']="${serial['viewout_name']} - ".($serial__show_season ? "${serial['season']} сезон " : "")."${search_film['series_text']} $serial__series_word";
