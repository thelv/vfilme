<?php
	...
	function preparevkiframe(&$iframe,$watch,$film,$blocked)
	{
		//return str_replace('</iframe>',"\r\n<a href='http://vkontakte.ru'>Смотреть \"${film}\"  ВКонтакте</a>\r\n</iframe>",$iframe);
		if($blocked)
		{
			$iframeSrc=preg_replace('/\A.*"(http.*)".*\Z/U', '$1', $iframe);
			$iframe=
			"
				<div 
					style='font-size:30px;color:red;width:480px;height:350px;background:#aaa; /*url(img/play.png) 50% 50% no-repeat*/;border:1px solid gray;cursor:pointer'
					onclsick='window.open(\"{$iframeSrc}\", \"vk_iframe\",\"height=360,width=607,left=283,bottom=50,top=\"+(/*$(window).height()-420*/245)+\"\")'
				>видео удалено
				</div>
			";
			return '480';
