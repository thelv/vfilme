...
<h1>
	...
	<?
		...
		{
			?>
				/ <? //<a href='<?= url('serial', $serial, $serie['season']) *>'> ?> $0url[season] сезон <? //</a> ?>
			<?
		}
		...
		{
			?>
				<? //<a href='<?= url('serial', $serial, $serie['season']) *>'> ?> $0url[serie] серия <? //</a> ?>
			<?
		}
	?>
</h1>
...
<div id="addToFavorites"
	onclick="					
		profile.addToFavorites($0serial['id'], ! profile.isFavorite($0serial['id']));
		$('#addToFavoritesImage').attr('src', profile.isFavorite($0serial['id']) ? '/serials/img/fav.png' : '/serials/img/fav_hide.png');
	"
>				
	<script>
		if(profile.isFavorite($0serial[id]))
		{
			...
			<?
				...
				{	
					if($season_==$serie['season']){	
						?>
							<a class='seriesLink' id='activeSeriesLink' href='<?= url('serial', $serial, $season_) ?>' onclick='return false;'>$$season_</a>
						<?
					}else{				
						?>
							<a class='seriesLink' href='<?= url('serial', $serial, $season_) ?>'>$$season_</a>
						<?				
					}
				}
			...
			<?				
				...
				foreach($seasons[$serie['season']] as $serie_=>$videos_)
				{
					...
					if($serie['serie']==$serie_){
						?>
							<a class='seriesLink $$classes' id='activeSeriesLink' href='<?= url('serial', $serial, $serie['season'], $serie_) ?>' onclick='return false;'>$$serie_</a>
						<?
					}else{
						?>
							<a class='seriesLink $$classes' href='<?= url('serial', $serial, $serie['season'], $serie_) ?>'>$$serie_</a>
						<?	
					}
				}
				...
				else
				{
					...
					{		
						if($video_['id']==$video['id']){
							?>
								<a class="seriesLink" id="activeSeriesLink" href='<?= url('serial', $serial, $serie['season'], $serie['serie'], $video_['id']) ?>' onclick='return false;'>$$i</a>
							<?
						}else{
							?>
								<a class="seriesLink" href='<?= url('serial', $serial, $serie['season'], $serie['serie'], $video_['id']) ?>'>$$i</a>
							<?	
						}
						$i++;
					}
