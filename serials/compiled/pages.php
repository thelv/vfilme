		...
		switch($type):
			...
			//search	
				...
				case 'search':
					...
					foreach($ids as $id) sel($dat, 'serial where=<`id`='.(int)($id).'>');
					$template='serials';
					break;
		...
		//pages
			...
			//serial
				...
				case 'serial':
					...
					sel($dat, '
						serial where=<`alias`=\''.mysql_real_escape_string($url['title']).'\'> limit=1
							serie order_by=<season, serie> where=<serie<>0> video limit=1!!
							serie where=<season='.(int)($season).' and serie='.(int)($serie).'> limit=1 video
