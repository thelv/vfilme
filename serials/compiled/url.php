<?php
	...
	function url($page, $p1, $p2, $p3, $p4, $p5){
		...
		global $globals; extract($globals);

		switch($page){
					
			case 'serials':
			
				return "/{$SERIALI}".($p1 ? '?p='.(int)($p1).'' : '');
			...
			case 'serial':
				...
				$serie=$p3;
				$video=$p4;
				return '/'.($SERIALI).'/'.urlencode($alias).''.($season ? '/'.(int)($season).'_'.($SEZON).''.($serie ? '_'.(int)($serie).'_'.($SERIA).'' : '') : '').($video ? '?v='.(int)($video).'' : '');
			...
			case 'search':
				...
				return '/'.($SERIALI).'?q='.urlencode($p2).''.($p1 ? '&p='.(int)($p1).'' : '');
					
			case 'ajax/serials':
			
				return '/serials/ajax/serials?q='.urlencode($p1).'&p='.(int)($p2).'';
			...
			case 'serialShort':
				...
				$serie=$p3;
				$video=$p4;
				return '/serials/'.(int)($id).''.($season ? '/'.(int)($season).'_season'.($serie ? '_'.(int)($serie).'_episode' : '') : '').($video ? '?v='.(int)($video).'' : '');
