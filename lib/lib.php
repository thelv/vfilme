<?php
	...
	function search_films($request,$n0,$n,&$films_number,$info='',$genre='')
	{
		...
		function RemoveRuEnd(&$words,$ruGlasn,$ruSogl)
		{
			...
			{
				$words[$key]=preg_replace("/(\A.+[$ruGlasn]{1}.*[$ruSogl]{1})(�|�|�|��|��|�|�|��|��|��|���|��|��|��|��|��)\z/iU",'$1',$word);
			}
		}
	...
	function get_serial_by_id($serial_id)
	{
		...
		$serial['name']=html_line_out($serial['name']);
		$serial['alias']=html_line_out($serial['alias']);
		mysql_free_result($res);
		return $serial;
	}
	
	function get_serial_by_alias($serial_alias)
	{
		$serial_alias=toCP1251($serial_alias);
		$dbquery_serial_alias=mysql_real_escape_string($serial_alias);
		//die("SELECT * FROM serial WHERE alias='$dbquery_serial_alias' LIMIT 1");
		$res=mysql_query("SELECT * FROM serial WHERE alias='$dbquery_serial_alias' LIMIT 1");
		$serial=mysql_fetch_assoc($res);
		$serial['name']=html_line_out($serial['name']);
		$serial['alias']=html_line_out($serial['alias']);
		mysql_free_result($res);
	...
	function get_series($serial,$season)
	{
		...
		$res=mysql_query("SELECT R.series AS series, R.series_text AS series_text, F.watch AS watch, F.img AS img, F.description AS description FROM season E, series R, film F WHERE E.serial=$dbquery_serial AND E.season=$dbquery_season AND R.season=E.Id AND F.watch=R.film ORDER BY R.series");
		while($film=mysql_fetch_assoc($res))
		{		
			$film['name']=html_line_out($film['name']);
			$film['description']=html_block_out($film['description']);
	...
	{	
        ...
        function get_serials_count()
        {
                $res=mysql_query('SELECT SQL_CACHE count(*) count FROM serial');
                $result=mysql_fetch_assoc($res);
                $count=$result['count'];
                mysql_free_result($res);
                return $count;
        ...
        /*function get_random_serials()
        {
                $count=get_serials_count();
                for($i=1;$i<=2;$i++)
                {
                        $n=rand(1, $count-1);
                        $res=mysql_query('SELECT * FROM serial LIMIT $n1, 1');
                        $serial=mysql_fetch_assoc($res);
                        mysql_free_result($res);
                        $serial['name']=toUTF($serial['name']);
                        $serial['alias']=toUTF($serial['alias']);
                        $serials[]=$serial;
                }                
                return $serials;
        }*/

        function get_serials($n0, $n)
        {
                $dbquery_n0=(int)$n0;
                $dbquery_n=(int)$n;
                $res=mysql_query("SELECT SQL_CACHE * FROM serial ORDER BY name LIMIT {$dbquery_n0}, {$dbquery_n}");
                echo mysql_error();
                while($serial=mysql_fetch_assoc($res))
                {
                        $serial['name']=toUTF($serial['name']);
                        $serial['alias']=toUTF($serial['alias']);
                        $serials[]=$serial;
